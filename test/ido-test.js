...
describe('Test IDO Token Contract', function () {
  ...
  beforeEach(async () => {
    ...
    await usdc.deployed()
    idoSale = await IDOSaleFactory.connect(owner).deploy(0, '1000000000000', '1000000000000', idoToken.address, usdc.address, 40, '10000000000000000', '20000000000000000', '20000000000000000', '100000000000000', owner.address)
  ...
  it('IDO Success', async () => {
    ...
    expect(idoSale.connect(minter).purchase('10000000000000000')).to.be.revertedWith('Insufficinet Fund')
    ...
    await (await idoSale.connect(minter).purchase('10000000000000000')).wait()
    expect((await idoSale.purchased(minter.address)).toString()).to.equal('10000000000000000')
    ...
    await (await usdc.connect(minter).approve(idoSale.address, 250)).wait()
    await (await idoSale.connect(minter).purchase('10000000000000000')).wait()
    expect((await idoSale.purchased(minter.address)).toString()).to.equal('20000000000000000')
    ...
    expect((await idoToken.balanceOf(minter.address)).toString()).to.equal('20000000000000000')
  ...
  it('IDO Fail', async () => {
    ...
    await (await idoSale.connect(minter).purchase('40000000000000000')).wait()
    expect((await idoSale.purchased(minter.address)).toString()).to.equal('40000000000000000')
